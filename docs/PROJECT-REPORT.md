...
(alpha = 100)** with a test RMSE of **$66,681** and **R² 0.667**. On Telco churn, the
champion is **Logistic Regression (C = 2.154)** with a test **ROC-AUC of 0.8448** and
accuracy of **0.811**. In both cases the interpretable model won — on churn it matched
the Random Forest's discrimination (0.8446 AUC) while being substantially more precise,
...
| **Rows (train/val/test)** | ~14,400 / ~3,100 / ~3,100 | 4,930 / 1,056 / 1,057 |
...
| **Champion** | **Ridge v001, alpha = 100** | **LogisticRegression v001, C = 2.154** |
| Validation | RMSE $64,787 · R² 0.680 | ROC-AUC 0.8455 · Accuracy 0.808 |
| Test (opened once) | RMSE $66,681 · R² 0.667 | ROC-AUC 0.8448 · Accuracy 0.811 |
| Runner-up | Lasso / LinearRegression, RMSE $64,887 | RandomForest, ROC-AUC 0.8446 |

### 10.2 California housing

The champion is Ridge with `alpha = 100`, selected on validation RMSE. Test performance
is RMSE $66,681, MAE $48,731, R² 0.667, MAPE 28.3%.

**The margin is small and is reported as such.** Ridge beat plain LinearRegression by
approximately $100 of validation RMSE — $64,787 against $64,887. Lasso landed at the same
$64,887. Three models therefore sit within $100 of each other. L2 shrinkage helped
...
The validation-to-test movement — $64,787 to $66,681 — is a modest and expected
...
The champion is Logistic Regression with `C = 2.154`, `solver = liblinear`,
`max_iter = 2000`, trained on 4,930 rows and evaluated on 1,057.

| Model | Val ROC-AUC | Val precision | Val recall | Val F1 |
|---|---|---|---|---|
| **LogisticRegression** | **0.8455** | **0.6498** | 0.5964 | 0.6220 |
| RandomForestClassifier | 0.8446 | 0.5011 | **0.8357** | 0.6265 |
...
(0.8446 versus 0.8455 — a difference well inside noise) but at a very different operating
...
Test performance is ROC-AUC 0.8448, accuracy 0.811, precision 0.687, recall 0.530,
F1 0.598. **The validation-to-test movement is 0.8455 to 0.8448** — essentially nil,
...
|---|---|---|
| `InternetService` | **+0.1208** | The model flags one group about 12 percentage points more often than its real churn rate justifies |
...
| 2 | `MonthlyCharges` | 0.0499 |
| 3 | `TotalCharges` | 0.0201 |
...
| Maximum calibration error (MCE) | 0.1806 |
| Mean predicted probability | 0.2566 |
| Actual base rate | 0.2658 |
| Global bias | -0.0093 |
...
**Accuracy is modest on housing.** R² 0.667 means a third of the variance in house values
